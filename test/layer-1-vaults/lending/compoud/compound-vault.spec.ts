...
import { faucet } from '@openzeppelin/hardhat-truffle5';
...
describe('CompoundVault', async () => {
  it('creates a new clone of a CompoundLendingVault and checks creation Event and correct owner', async () => {
    const data: string = abiCoder.encode(['address', 'uint256', 'address', 'address', 'address', 'address'], [USDC_ADDRESS, BORROW_RATE, CETH_ADDRESS, ASSET_PRICE_FEED_ADDRESS, BORROW_ASSET_PRICE_FEED_ADDRESS, deployerAddress]);
